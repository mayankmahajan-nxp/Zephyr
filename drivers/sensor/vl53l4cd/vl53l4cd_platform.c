/* vl53l4cd_platform.c - Zephyr customization of ST vl53l4cd library.
...
LOG_MODULE_DECLARE(VL53L4CD, CONFIG_SENSOR_LOG_LEVEL);
				 ...
				 uint8_t AndData, uint8_t OrData)
{
	...
	status_int = VL53L4CD_RdByte(Dev, index, &data);
	if (status_int < 0) {
		Status = VL53L4CD_ERROR_CONTROL_INTERFACE;
		LOG_ERR("VL53L4CD_RdByte failed (%d)", Status);
	}

	if (Status == VL53L4CD_ERROR_NONE) {
		data = (data & AndData) | OrData;
		status_int = VL53L4CD_WrByte(Dev, index, data);
		if (status_int != 0) {
			Status = VL53L4CD_ERROR_CONTROL_INTERFACE;
			LOG_DBG("VL53L4CD_WrByte failed.(%d)", Status);
		}
	}
