...
static int modem_ubx_process_received_byte(struct modem_ubx *ubx, uint8_t byte)
{
	...
	static uint16_t recv_ubx_frame_len;
	...
	if (ubx->received_ubx_preamble_sync_chars == false) {
		...
		if (ubx->work_buf_len == UBX_FRM_HEADER_SZ) {
			uint16_t recv_ubx_payload_len = ubx->work_buf[UBX_FRM_PAYLOAD_SZ_H_IDX];

			recv_ubx_payload_len = ubx->work_buf[UBX_FRM_PAYLOAD_SZ_H_IDX] << 8;
			recv_ubx_payload_len |= ubx->work_buf[UBX_FRM_PAYLOAD_SZ_L_IDX];

			recv_ubx_frame_len = recv_ubx_payload_len + UBX_FRM_SZ_WITHOUT_PAYLOAD;
		}

		if (ubx->work_buf_len == recv_ubx_frame_len) {
